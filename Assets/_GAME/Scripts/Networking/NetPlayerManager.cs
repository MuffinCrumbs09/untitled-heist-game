...
public class NetPlayerManager : NetworkBehaviour
{
    ...
    public void SetPlayerStateServerRpc(PlayerState newState, RpcParams rpc = default)
    {
        ...
        for (int i = 0; i < playerData.Count; i++)
        {
            ...
            if (temp.CLIENTID.Equals(targetID))
            {
                temp.STATE = newState;
                playerData[i] = temp;
                Debug.Log(playerData[i].USERNAME + " is now " + playerData[i].STATE);
            }
        }
    }
    ...
    public void SavePlayerDataToDesktop()
    {
        ...
        {
            Debug.LogWarning("NetPlayerManager: No player data to save.");
            return;
        }
        ...
        Debug.Log($"NetPlayerManager: Player data saved to {filePath}");
    }
    #endregion
}
